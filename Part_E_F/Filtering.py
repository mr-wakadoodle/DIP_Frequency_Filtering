...
def main():
    ...
    if filter_name == 'Butterworth_HPF':
        mask = Butterworth_HighPass_Filter(image,cutoff_frequency,int(sys.argv[4]))
    elif filter_name == 'Butterworth_LPF':
        mask = Butterworth_LowPass_Filter(image,cutoff_frequency,int(sys.argv[4]))
    ...
    Filtered_Image = 'Part_E_F/Output/' + filter_name + '_Filtered_Image' + ".jpg"
    cv2.imwrite(Filtered_Image, output[0])
    DFT = 'Part_E_F/Output/' + filter_name +  '_DFT' + ".jpg"
    cv2.imwrite(DFT, output[1])
    Filtered_DFT = 'Part_E_F/Output/' + filter_name +  '_Filtered_DFT' + ".jpg"
    cv2.imwrite(Filtered_DFT, output[2])


    # print(Ideal_HighPass_Filter(image,int(sys.argv[2])))
    # Ideal_LowPass_Filter(sys.args[0], sys.args[1])
